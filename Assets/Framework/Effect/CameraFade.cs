...
public class CameraFade : MonoBehaviour
{
    ...
    //淡入
    public static void FadeIn(float fadeTime, CallBack completeCallBack = null, bool isForceAlpha = false,
        float delay = 0)
    {
    ...
    //淡出
    public static void FadeOut(float fadeTime, CallBack completeCallBack = null, bool isForceAlpha = false,
        float delay = 0)
    {
        CameraFadeData data =
            GetCameraFadeData(CameraFadeType.FadeOut, isForceAlpha, delay, fadeTime, completeCallBack);
        Instance.StartFadeFunc(data);
    }

    //从淡入到淡出
    public static void FadeInToOut(float _fadeInTime, float afterInDelayTime, float _fadeOutTime,
        CallBack afterFadeInCallback = null, CallBack afterFadeOutCallback = null, float delay = 0,
        bool isForceAlpha = false)
    {
        CameraFadeData data =
            GetCameraFadeData(CameraFadeType.FadeIn, isForceAlpha, delay, _fadeInTime, afterFadeInCallback);
        CameraFadeData data2 = GetCameraFadeData(CameraFadeType.FadeOut, isForceAlpha, afterInDelayTime, _fadeOutTime,
    ...
    private static CameraFadeData GetCameraFadeData(CameraFadeType fadeType, bool isForceAlpha, float delay,
        float fadeTime, CallBack completeCallBack)
    {
    ...
    public class CameraFadeData
    {
        ...
        public CameraFadeData(CameraFadeType fadeType, bool isForceAlpha, float delay, float fadeTime,
            CallBack completeCallBack)
        {
            SetCameraFadeData(fadeType, isForceAlpha, delay, fadeTime, completeCallBack);
        }

        public void SetCameraFadeData(CameraFadeType fadeType, bool isForceAlpha, float delay, float fadeTime,
            CallBack completeCallBack)
        {
