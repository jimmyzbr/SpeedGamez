...
public class AnimData
{
    ...
    public void Init()
    {
        ...
        {
            case AnimType.UGUI_Color:
                UguiColorInit(m_isChild);
                break;
            case AnimType.UGUI_Alpha:
                UguiAlphaInit(m_isChild);
                break;
            case AnimType.UGUI_AnchoredPosition:
                UguiPositionInit();
                break;
    ...
    public void BezierInit()
    {
        if (m_v3Contral == null)
        {
            if (m_floatContral != null)
            {
                ...
                {
                    m_v3Contral[0] = UnityEngine.Random.insideUnitSphere * m_floatContral[0] +
                                     (m_fromV3 + m_toV3) * 0.5f;
                }
            }
            else
            {
    ...
    {
        return (1 - n_time) * (1 - n_time) * startPos + 2 * (1 - n_time) * n_time * v3_ControlPoint[0] +
               n_time * n_time * endPos;
    }
    ...
    public void ShowDebug()
    {
        ...
        for (int i = 0; i < infos.Length; i++)
        {
            ...
            {
                Debug.Log(" " + infos[i].Name + " is not equal ! this value : " + infos[i].GetValue(this) +
                          " data Value :" + infos[i].GetValue(data));
            }
        }
    }
