...
public class BundleConfigEditorWindow : EditorWindow
{
    ...
    void RelyPackagesView()
    {
        for (int i = 0; i < relyPackages.Count; i++)
        {
            ...
            if (relyPackages[i].isFold)
            {
                ...
                relyPackages[i].isCollectDependencies =
                    EditorGUILayout.Toggle("收集依赖", relyPackages[i].isCollectDependencies);
    ...
    void LogView()
    {
        ...
        for (int i = 0; i < relyPackages.Count; i++)
        {
            ...
            {
                EditorGUILayout.LabelField("ERROR: " + relyPackages[i].errorMsg[j],
                    EditorGUIStyleData.ErrorMessageLabel);
            }
        }
    ...
    void NewPackage()
    {
        ...
        BuildPipeline.BuildAssetBundles(Application.dataPath + "/StreamingAssets/", BuildAssetBundleOptions.None,
            PackageService.GetTargetPlatform);
...
{
    [System.NonSerialized] public Object obj;
    public string path;
}
