...
public static class PackageService
{
    ...
    static BuildAssetBundleOptions
        relyBuildOption = BuildAssetBundleOptions.AppendHashToAssetBundleName; //每次二进制一致  //依赖包打包设置   
    ...
    static BuildAssetBundleOptions bundleBuildOption =
        BuildAssetBundleOptions.DeterministicAssetBundle //每次二进制一致  //Bundle打包设置
        | BuildAssetBundleOptions.CollectDependencies //收集依赖
        | BuildAssetBundleOptions.CompleteAssets; //完整资源
    ...
    public static IEnumerator Package(List<EditPackageConfig> relyPackages, List<EditPackageConfig> bundles,
    ...
    static void SetAssetsBundleName(string path)
    {
        ...
        //移除文件夹中的下划线
        //移除空格
        string name = FileTool.RemoveExpandName(path).ToLower().Replace("/_", "/").Replace("assets/", "")
            .Replace(" ", "");
