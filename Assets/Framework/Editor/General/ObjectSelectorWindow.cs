...
public class ObjectSelectorWindow : EditorWindow
{
    ...
    public static void Show(EditorWindow otherWindow, string assetName, string[] paths, Type assetType,
        CallBack<string, UnityEngine.Object> selectFileCallBack)
    {
        if (win == null)
    ...
    private void Init(EditorWindow otherWindow, string assetName, string[] paths, Type assetType,
        CallBack<string, UnityEngine.Object> selectFileCallBack)
    {
        ...
        foreach (var id in tempGIDs)
        {
            ...
            oa.previewIcon =
                oa.previewEditor.RenderStaticPreview(p, null, (int)maxGridSize,
                    (int)maxGridSize); //AssetPreview.GetAssetPreview(obj);
    ...
    {
        GUILayout.BeginArea(new Rect(0, position.height - selectAreaSizeHeight, position.width, selectAreaSizeHeight),
            m_Styles.previewTextureBackground);
        if (selectAsset != null)
        {
            GUILayout.BeginHorizontal();
            GUILayout.Label(selectAsset.previewIcon, GUILayout.Width(selectAreaSizeHeight - 5),
                GUILayout.Height(selectAreaSizeHeight - 5));
    ...
    {
        GUILayout.BeginArea(new Rect(0, 44, position.width,
            position.height - 24f - m_ToolbarHeight - selectAreaSizeHeight));
