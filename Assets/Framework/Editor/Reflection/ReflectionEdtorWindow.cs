...
public class ReflectionEdtorWindow : EditorWindow
{
        ...
        for (int i = 0; i < m_asb.Length; i++)
        {
            if (m_searchAssemblyContent == "" ||
                (m_searchAssemblyContent != "" &&
                 m_asb[i].FullName.ToLower().Contains(m_searchAssemblyContent.ToLower())))
            {
                AssemvlyGUI(m_asb[i]);
            }
        }
            ...
            for (int i = 0; i < types.Length; i++)
            {
                if (types[i].IsClass)
                {
                    if (m_searchClassContent == "" ||
                        (m_searchClassContent != "" &&
                         types[i].Name.ToLower().Contains(m_searchClassContent.ToLower())))
                    {
    ...
    void AllMethodGUI()
    {
        if (m_currentClass != null)
        {
            ...
            MethodInfo[] typesTmp = m_currentClass.GetMethods(BindingFlags.Public | BindingFlags.NonPublic |
                                                              BindingFlags.Static | BindingFlags.Instance);

            for (int i = 0; i < typesTmp.Length; i++)
            {
                if (m_searchMethodContent == "" ||
                    (m_searchMethodContent != "" &&
                     typesTmp[i].Name.ToLower().Contains(m_searchMethodContent.ToLower())))
                {
    ...
    void MethodGUI(MethodInfo method)
    {
        ...
        content3 += "<color=#11FF11>" + GetTypeName(method.ReturnType) + "</color> <color=white>" + method.Name +
                    "</color>";
        ...
        {
            content3 += GetParmPrdfix(infos[i], true) + "<color=#11FF11>" + GetTypeName(infos[i].ParameterType) +
                        "</color> <color=#FF77FF>" + infos[i].Name + "</color>" + GetParmPostfix(infos[i], true);
    ...
    string GenerateCallCode(Assembly ab, Type type, MethodInfo method)
    {
        ...
        for (int i = 0; i < infos.Length; i++)
        {
            content += GetParmPrdfix(infos[i], false) + GetTypeName(infos[i].ParameterType) + " " + infos[i].Name +
                       GetParmPostfix(infos[i], false);
    ...
    void AllFieleGUI()
    {
        if (m_currentClass != null)
        {
            ...
            FieldInfo[] typesTmp = m_currentClass.GetFields(BindingFlags.Public | BindingFlags.NonPublic |
                                                            BindingFlags.Static | BindingFlags.Instance);
    ...
    void AllPropertyGUI()
    {
        if (m_currentClass != null)
        {
            ...
            PropertyInfo[] typesTmp = m_currentClass.GetProperties(BindingFlags.Public | BindingFlags.NonPublic |
                                                                   BindingFlags.Static | BindingFlags.Instance);
    ...
    void PropertyGUI(PropertyInfo info)
    {
        ...
        content2 += "<color=#11FF11>" + GetTypeName(info.PropertyType) + "</color> <color=white>" + info.Name +
                    "</color>";
