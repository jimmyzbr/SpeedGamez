...
public class ImportTool : AssetPostprocessor
{
    ...
    static void OnImportFBX(string assetPath)
    {
        ...
        try
        {
            ...
            {
                Debug.LogWarning("FBXImportTool: " + assetPath + " 资源不满足 FBX" + c_FBXspltChar + "模型名(下划线分级)" +
                                 c_FBXspltChar + "动作名" + c_FBXspltChar + "分层数(可选)  的格式，不能被自动导入");
            }
        }
        catch (Exception e)
        {
    ...
    private static void AddStateTransition(string animName, string path, AnimatorControllerLayer layer)
    {
        ...
        if (state == null)
        {
            Vector3 pos = new Vector3(((sm.states.Length / colCount) + 1) * xSpace,
                ((sm.states.Length % colCount) + 4) * ySpace);
