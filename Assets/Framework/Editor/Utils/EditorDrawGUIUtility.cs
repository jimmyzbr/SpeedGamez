...
public static class EditorDrawGUIUtility
{
    ...
    /// <returns></returns>
    public static Rect DrawCanDragArea(Rect dragScreenRect, Rect maxScreenRect, CallBack drawCallBack,
        EditorCanDragAreaSide canDragAreaSide = EditorCanDragAreaSide.Top | EditorCanDragAreaSide.Bottom |
                                                EditorCanDragAreaSide.Left | EditorCanDragAreaSide.Right,
        string style = "")
    {
        ...
        if ((canDragAreaSide & EditorCanDragAreaSide.Left) > 0)
            dragScreenRect.xMin = EditorDrawGUIUtility.DrawHorizontalDragSplitter(dragScreenRect.xMin,
                maxScreenRect.xMin, dragScreenRect.yMin, 0, 5, maxScreenRect.width, dragScreenRect.height);
        else
            dragScreenRect.xMin = maxScreenRect.xMin;

        if ((canDragAreaSide & EditorCanDragAreaSide.Right) > 0)
            dragScreenRect.xMax = EditorDrawGUIUtility.DrawHorizontalDragSplitter(dragScreenRect.xMax,
                dragScreenRect.xMin, dragScreenRect.yMin, 5, 0, maxScreenRect.width, dragScreenRect.height);
        else
            dragScreenRect.xMax = maxScreenRect.xMax;

        if ((canDragAreaSide & EditorCanDragAreaSide.Top) > 0)
            dragScreenRect.yMin = EditorDrawGUIUtility.DrawVerticalDragSplitter(dragScreenRect.yMin, maxScreenRect.yMin,
                dragScreenRect.xMin, 0, 5, maxScreenRect.height, dragScreenRect.width);
        else
            dragScreenRect.yMin = maxScreenRect.yMin;

        if ((canDragAreaSide & EditorCanDragAreaSide.Bottom) > 0)
            dragScreenRect.yMax = EditorDrawGUIUtility.DrawVerticalDragSplitter(dragScreenRect.yMax,
                dragScreenRect.yMin, dragScreenRect.xMin, 5, 0, maxScreenRect.height, dragScreenRect.width);
        else
    ...
    public static float DrawHorizontalDragSplitter(float dragX, float minX, float pos_Y, float minLeftSide,
        float minRightSide, float width, float splitterHeight)
    {
    ...
    /// <returns></returns>
    public static float DrawVerticalDragSplitter(float dragY, float minY, float pos_X, float minTopSide,
        float minBottomSide, float height, float splitterWidth)
    {
    ...
    public static void DrawHorizontalSplitter(Rect dragRect)
    {
        if (Event.current.type == EventType.Repaint)
        {
            ...
            Color b = (!EditorGUIUtility.isProSkin)
    ...
    public static Rect HandleHorizontalSplitter(Rect dragRect, float minX, float width, float minLeftSide,
        float minRightSide)
    {
        if (Event.current.type == EventType.Repaint)
        {
    ...
    public static Rect HandleVerticalSplitter(Rect dragRect, float minY, float height, float minTopSide,
        float minBottomSide)
    {
        if (Event.current.type == EventType.Repaint)
        {
