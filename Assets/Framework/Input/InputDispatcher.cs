...
{
    protected Dictionary<string, InputEventHandle<Event>> m_Listeners =
        new Dictionary<string, InputEventHandle<Event>>();
    ...
    /// </summary>
    Dictionary<InputEventHandle<IInputEventBase>, InputEventHandle<Event>> m_ListenerHash =
        new Dictionary<InputEventHandle<IInputEventBase>, InputEventHandle<Event>>();

    public override void AddListener(string eventKey, InputEventHandle<IInputEventBase> callBack)
    {
        InputEventHandle<Event> temp = (inputEvent) => { callBack((IInputEventBase)inputEvent); };
    ...
    void DispatchSingleEvent(Event inputEvent, InputEventHandle<Event> callBack)
    {
        if (callBack != null)
        {
            ...
            {
                Debug.LogError("DispatchSingleEvent Name: " + typeof(Event).ToString() + " key: " +
                               inputEvent.EventKey + " Exception: " + e.ToString());
            }
        }
    }
}