...
public class LanguageDataEditorWindow : EditorWindow
{
    ...
    void MergeLanguage(string path, SystemLanguage language)
    {
        ...
        string languageKey = FileTool.GetFileNameByPath(FileTool.RemoveExpandName(path))
            .Replace(LanguageManager.c_DataFilePrefix + language + "_", "").Replace("_", "/");
        ...
        foreach (var key in aimLanguage.TableIDs)
        {
            ...
            else
            {
                ...
                {
                    Debug.Log("更新字段 key" + key + " Value >" + localLanguage[key].GetString(LanguageManager.c_valueKey) +
                              "< newValue >" + value + "<");
    ...
    public Dictionary<string, string> CreateLanguageNewFile(SystemLanguage language, string fullKeyFileName,
