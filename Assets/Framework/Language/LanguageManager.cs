...
public class LanguageManager
{
    ...
    private static SystemLanguage SetConfig()
    {
        ...
        Debug.Log("config.useSystemLanguage:" + config.useSystemLanguage + " config.defaultLanguage:" +
    ...
    public static bool ContainsFullKeyName(string fullKeyName)
    {
        ...
        else
        {
            ...
            foreach (var item in data.TableIDs)
            {
                ...
                {
                    Debug.LogError("Find:" + fullKeyName + "\n ContainsFullKeyName Error (" + fullFileName + "/" +
                                   item + ") -> (" + data[item].GetString(c_valueKey) + ")\n" + e);
                }
            }
    ...
    public static string GetContentByKey(string fullKeyName, params object[] contentParams)
    {
        ...
        try
        {
            ...
            if (ApplicationManager.Instance != null && ApplicationManager.Instance.showLanguageValue &&
                ApplicationManager.Instance.m_AppMode == AppMode.Developing)
            {
    ...
    {
        bool isChinese = LanguageManager.CurrentLanguage == SystemLanguage.ChineseSimplified ||
                         LanguageManager.CurrentLanguage == SystemLanguage.ChineseTraditional ||
                         LanguageManager.CurrentLanguage == SystemLanguage.Chinese;
        return isChinese;
    }
}