...
public class AssetsLoadController
{
    ...
    public AssetsData LoadAssets<T>(string path) where T : Object
    {
        return LoadAssetsLogic(path,
            ...
            ,
            (p) => { return loader.LoadAssets<T>(p); });
    }

    public AssetsData LoadAssetsLogic(string path, CallBackR<bool> checkContainsAssets,
        CallBackR<AssetsData, string> loadMethod)
    {
