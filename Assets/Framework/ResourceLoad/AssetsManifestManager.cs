...
public class AssetsManifestManager
{
    ...
    public static void LoadAssetsManifest()
    {
        ...
        if (RecordManager.GetData(HotUpdateManager.c_HotUpdateRecordName)
            .GetRecord(HotUpdateManager.c_useHotUpdateRecordKey, false))
        {
    ...
    private static void LoadDependenciePaths()
    {
        ...
        for (int i = 0; i < sArr.Length; i++)
        {
            ...
            dependenciePathsDic.Add(ResourcesConfigManager.GetLoadPathBase(ResourceManager.LoadType, assetPath),
                dependens);
        }
    ...
    public static string[] GetAllDependenciesPaths(string path)
    {
        ...
        {
            Debug.LogError("没找到依赖 GetAllDependenciesName.Name :" + path + " dependencieNamesDic=>" +
                           dependenciePathsDic.Count);
            return new string[0];
        }
    }
