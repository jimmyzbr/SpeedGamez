...
public class DevelopReplayManager
{
    ...
    static void LoadEventStream(string[] content)
    {
        ...
        for (int i = 0; i < content.Length; i++)
        {
            if (content[i] != "")
            {
                ...
                IInputEventBase eTmp = (IInputEventBase)Deserializer.Deserialize(
                    Type.GetType(info[c_eventNameKey].ToString()), info[c_serializeInfoKey].ToString());
    ...
    static Rect windowRect =
        new Rect(Screen.width * 0.2f, Screen.height * 0.05f, Screen.width * 0.6f, Screen.height * 0.9f);
        ...
        for (int i = 0; i < FileNameList.Length; i++)
        {
            ...
            else
            {
                if (GUILayout.Button("上传 " + FileNameList[i]))
                {
                    ...
                    HTTPTool.Upload_Request_Thread(URLManager.GetURL("ReplayFileUpLoadURL"), replayPath,
                        UploadCallBack);
                    HTTPTool.Upload_Request_Thread(URLManager.GetURL("ReplayFileUpLoadURL"), randomPath,
                        UploadCallBack);
                }
            }
        }
    ...
    static void ShowLogList()
    {
        ...
        if (GUILayout.Button("清除日志"))
        {
            ...
            {
                Debug.Log("已删除所有日志");
                FileTool.SafeDeleteDirectory(PathTool.GetAbsolutePath(ResLoadLocation.Persistent,
                    LogOutPutThread.LogPath));
                FileNameList = new string[0];
            });
        }
    ...
    static void ShowPersistentFileList()
    {
        ...
        if (GUILayout.Button("清除持久数据文件"))
        {
            OpenWarnWindow("确定要删除所有持久数据文件吗？", () =>
            {
                ...
                FileTool.SafeDeleteDirectory(PathTool.GetAbsolutePath(ResLoadLocation.Persistent,
                    PersistentFileManager.c_directoryName));
                FileNameList = new string[0];
            });
        }
    ...
    public static string[] GetRelpayFileNames()
    {
        ...
        string[] allFileName =
            Directory.GetFiles(PathTool.GetAbsolutePath(ResLoadLocation.Persistent, c_directoryName));
