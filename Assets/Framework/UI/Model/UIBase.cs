...
public class UIBase : MonoBehaviour, UILifeCycleInterface
{
    ...
    public void AddOnClickListenerByCreate(Button button, string compName,
        InputEventHandle<InputUIOnClickEvent> callback, string parm = null)
    {
        InputButtonClickRegisterInfo info =
            InputUIEventProxy.GetOnClickListener(button, UIEventKey, compName, parm, callback);
        info.AddListener();
        m_OnClickEvents.Add(info);
    }

    public void AddLongPressListener(string compName, InputEventHandle<InputUILongPressEvent> callback,
        string parm = null)
    {
        InputEventRegisterInfo<InputUILongPressEvent> info =
            InputUIEventProxy.GetLongPressListener(GetLongPressComp(compName), UIEventKey, compName, parm, callback);
        info.AddListener();
        m_LongPressEvents.Add(info);
    }

    public void AddBeginDragListener(string compName, InputEventHandle<InputUIOnBeginDragEvent> callback,
        string parm = null)
    {
        InputEventRegisterInfo<InputUIOnBeginDragEvent> info =
            InputUIEventProxy.GetOnBeginDragListener(GetDragComp(compName), UIEventKey, compName, parm, callback);
        info.AddListener();
        m_BeginDragEvents.Add(info);
    }

    public void AddEndDragListener(string compName, InputEventHandle<InputUIOnEndDragEvent> callback,
        string parm = null)
    {
        InputEventRegisterInfo<InputUIOnEndDragEvent> info =
            InputUIEventProxy.GetOnEndDragListener(GetDragComp(compName), UIEventKey, compName, parm, callback);
        info.AddListener();
    ...
    public void AddOnDragListener(string compName, InputEventHandle<InputUIOnDragEvent> callback, string parm = null)
    {
        InputEventRegisterInfo<InputUIOnDragEvent> info =
            InputUIEventProxy.GetOnDragListener(GetDragComp(compName), UIEventKey, compName, parm, callback);
        info.AddListener();
    ...
    public InputButtonClickRegisterInfo GetClickRegisterInfo(string buttonName,
        InputEventHandle<InputUIOnClickEvent> callback, string parm)
    {
    ...
    public void RemoveOnClickListener(string buttonName, InputEventHandle<InputUIOnClickEvent> callback,
        string parm = null)
    {
    ...
    public void RemoveLongPressListener(string compName, InputEventHandle<InputUILongPressEvent> callback,
        string parm = null)
    {
    ...
    public InputEventRegisterInfo<InputUILongPressEvent> GetLongPressRegisterInfo(string compName,
        InputEventHandle<InputUILongPressEvent> callback, string parm)
    {
        string eventKey = InputUILongPressEvent.GetEventKey(UIName, compName, parm);
        for (int i = 0; i < m_LongPressEvents.Count; i++)
        {
            InputEventRegisterInfo<InputUILongPressEvent> info =
                (InputEventRegisterInfo<InputUILongPressEvent>)m_LongPressEvents[i];
            if (info.eventKey == eventKey
                && info.callBack == callback)
            {
                return info;
        ...
        throw new Exception("GetLongPressRegisterInfo Exception not find RegisterInfo by " + compName + " parm " +
                            parm);
    }
    ...
    {
        var model = CreateModelShow(modelName, layerName, orthographic, orthographicSize, backgroundColor,
            localPosition, localScale, eulerAngles, texSize, nearClippingPlane, farClippingPlane);
    ...
    void DisposeLifeComponent()
    {
        for (int i = 0; i < m_lifeComponent.Count; i++)
        {
            ...
            {
                Debug.LogError("UIBase DisposeLifeComponent Exception -> UIEventKey: " + UIEventKey + " Exception: " +
                               e.ToString());
            }
        }
    ...
    protected Dictionary<GameObject, GuideHeightLightComponent> m_CreateCanvasDict =
        new Dictionary<GameObject, GuideHeightLightComponent>(); //保存Canvas的创建状态
    ...
    public GameObject GetGuideDynamicCreateItem(string itemName)
    {
        ...
        if (strArr.Length > 0)
        {
            ...
            else
            {
                ...
                Debug.Log(
                    "UIBase : Index :" + index + "  firstName :" + firstName + " m_ChildList:" + m_ChildList.Count);
                int tempIndex = 0;
                for (int i = 0; i < m_ChildList.Count; i++)
                {
    ...
    {
        Vector3 scale = UIManager.UILayerManager.GetUICameraDataByKey(cameraKey).m_root.GetComponent<RectTransform>()
            .localScale;
