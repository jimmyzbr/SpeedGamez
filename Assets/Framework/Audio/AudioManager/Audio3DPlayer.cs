...
{
    public Dictionary<GameObject, Dictionary<int, AudioAsset>> bgMusicDic =
        new Dictionary<GameObject, Dictionary<int, AudioAsset>>();
    ...
    public AudioAsset PlayMusic(GameObject owner, string audioName, int channel = 0, bool isLoop = true,
        float volumeScale = 1, float delay = 0f, float fadeTime = 0.5f, string flag = "")
    {
        if (owner == null)
        {
    ...
    public void PlaySFX(Vector3 position, string name, float volumeScale = 1f, float delay = 0f)
    {
        ...
        if (ac)
            mono.StartCoroutine(PlaySFXIEnumerator(position, ac,
                AudioPlayManager.TotleVolume * AudioPlayManager.SFXVolume * volumeScale, delay));
    }
