...
public static class ParseTool
{
    ...
    private static string Array2String(object value, char[] arraySplitFormat, int charTagIndex)
    {
        ...
        {
            Debug.LogError("charTagIndex:" + charTagIndex + " arraySplitFormat:" + arraySplitFormat.Length + " value:" +
                           value.GetType() + "\n" + e);
            return result;
        }
    ...
    /// <returns></returns>
    private static object CreateInterleavedArray(string value, Type elementType, int num, char[] arraySplitFormat,
        int charTagIndex)
    {
        ...
        for (int i = 0; i < arrayLenth; i++)
        {
            ...
            {
                childValue = CreateInterleavedArray(tempStr, elementType, arraySplitFormat.Length - charTagIndex,
                    arraySplitFormat, charTagIndex);
            }
            else
            {
            ...
            {
                Debug.LogError(instance.GetType() + " instance.Length:" + instance.Length + " childValue.GetType():" +
                               childValue.GetType() + "\n" + e);
            }
        }
