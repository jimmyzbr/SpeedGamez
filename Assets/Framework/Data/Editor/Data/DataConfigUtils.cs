        ...
        switch (fieldValueType)
        {
            ...
            case FieldType.Enum:
                ...
                catch (Exception e)
                {
                    ...
                    Debug.LogError("fieldValueType:" + fieldValueType + " enumType:" + enumType + " type:" + type +
                                   " v:" + v + "\n" + e);
                    throw e;
                }
    ...
    public static void CreatDataCSharpFile(string dataName, DataTable data)
    {
        ...
        if (type.Count > 0)
        {
            for (int i = 1; i < data.TableKeys.Count; i++)
            {
                ...
                {
                    //访问类型 + 字段类型  + 字段名
                    content += "public " + OutPutFieldName(data.m_tableTypes[key], enumType, m_ArraySplitFormat) +
                               " m_" + key + note;
                }
                //默认字符类型
                else
        ...
        content += "\t\t{\n";
        content += "\t\t\tthrow new Exception(\"" + className +
                   " LoadData Exception Not Fond key ->\" + key + \"<-\");\n";
        content += "\t\t}\n";
        content += "\n";
        ...
        if (type.Count > 0)
        {
            for (int i = 1; i < data.TableKeys.Count; i++)
            {
                ...
                {
                    content += "m_" + key + " = data." +
                               OutPutFieldFunction(data.m_tableTypes[key], enumType, m_ArraySplitFormat) + "(\"" + key +
                               "\")";
                }
                //默认字符类型
                else
                {
                    content += "m_" + key + " = data." +
                               OutPutFieldFunction(FieldType.String, enumType, m_ArraySplitFormat) + "(\"" + key +
                               "\")";
