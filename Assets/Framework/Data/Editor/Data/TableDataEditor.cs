...
public class TableDataEditor
{
    ...
    public void Init(EditorWindow editorWindow)
    {
        ...
        {
            configFileNames.AddRange(PathUtils.GetDirectoryFileNames(m_directoryPath, new string[] { ".txt" }, false,
                false));
        }
    ...
    void ChooseFile()
    {
        ...
        if (!string.IsNullOrEmpty(chooseFileName) && GUILayout.Button("删除", GUILayout.Width(60)))
        {
            ...
            {
                File.Delete(Application.dataPath + "/Resources/" + DataManager.c_directoryName + "/" + chooseFileName +
                            ".txt");
            ...
            GeneralDataModificationWindow.OpenWindow(editorWindow, "新建配置文件", "", (value) =>
                {
                    ...
                    GeneralDataModificationWindow.otherParameter =
                        EditorDrawGUIUtil.DrawBaseValue("主键名:", GeneralDataModificationWindow.otherParameter);
    ...
    void DrawTableDataGUI()
    {
        ...
        Rect svPos_temp = new Rect(new Vector2(0, r.y),
            new Vector2(position.width - wWith, position.height - r.y - detaH));
        Vector2 v = GetCententSize();

        svPos = GUI.BeginScrollView(
            new Rect(svPos_temp.x + wWith, svPos_temp.y + detaH, svPos_temp.width, svPos_temp.height), svPos,
        ...
        //顶部格子
        topGridSVPos = GUI.BeginScrollView(new Rect(svPos_temp.x, svPos_temp.y, svPos_temp.width + wWith, detaH),
            topGridSVPos, new Rect(r.x, r.y, v.x + wWith, detaH), false, false, "Label", "Label");
        ...
        for (int i = 0; i < withItemList.Count; i++)
        {
            ...
            if (GUI.Button(new Rect(dragR.x, dragR.y + dragR.height / 3, dragR.width, dragR.height * 2 / 3),
                    i.ToString()))
            {
                if (i == 0)
                    continue;
        ...
        //默认第一列
        Debug.unityLogger.logEnabled = false;
        leftGridSVPos = GUI.BeginScrollView(new Rect(svPos_temp.x, tempHeight, wWith, svPos_temp.height), leftGridSVPos,
            new Rect(r.x, tempHeight, wWith, v.y), false, false, "Label", "Label");
        ...
        for (int i = 2; i < heightItemList.Count; i++)
        {
            ...
            else
            {
                ...
                if (GUI.Button(new Rect(dragR.x + firstLenth * 2, dragR.y, dragR.width - firstLenth * 2, dragR.height),
                        vStr))
                {
    ...
    private void GridTopFunctionGUI()
    {
        ...
        nowButtonFontSize =
            EditorGUILayout.IntSlider("字体大小", nowButtonFontSize, oldButtonFontSize / 2, MaxButtonFontSize);
        GUILayout.FlexibleSpace();
        ...
        GeneralDataModificationWindow.OpenWindow(editorWindow, "将字段数据转换为多语言数据", "", (value) =>
            {
                ...
                GeneralDataModificationWindow.otherParameter =
                    EditorDrawGUIUtil.DrawBaseValue("多语言文件名:", GeneralDataModificationWindow.otherParameter);
        ...
        for (int i = heightStartIndex; i < heightEndIndex; i++)
        {
            ...
            for (int j = 0; j < withItemList.Count; j++)
            {
                ...
                if (i == 0)
                {
                    ...
                    showStr = EditorDrawGUIUtil.GetFormatName(field,
                        DataConfigUtils.ConfigFieldValueType2Type(fieldValueType, enumType, m_ArraySplitFormat), "red");
                }
                else if (i == 1)
                {
                    GUI.color = Color.cyan;
                ...
                else
                {
                    ...
                    if (modofyValue == null && GUI.Button(rect, showStr, helpBoxStyle))
                    {
                        ...
                        if (i == 0)
                        {
                            ...
                            f.fieldName = field;
                            f.description = m_currentData.m_noteValue.ContainsKey(field)
                                ? m_currentData.m_noteValue[field]
                                : "";
                            f.fieldValueType = fieldValueType;
                            f.defultValue = DataConfigUtils.TableString2ObjectValue(defaultValue, fieldValueType,
                                enumType, m_ArraySplitFormat);
                            f.enumType = enumType;
                        ...
                        {
                            if (value == null)
                                value = DataConfigUtils.TableString2ObjectValue("", fieldValueType, enumType,
                                    m_ArraySplitFormat);
                        }

                        GeneralDataModificationWindow.OpenWindow(editorWindow, "修改数据", value, DrawModifiValueGUI,
                            CheckModifiValueCallBack, ModificationCompleteCallBack);
                    }
                    ...
                    if (modofyValue != null)
                    {
                        int index = i - 3;
                        string valueStr =
                            DataConfigUtils.ObjectValue2TableString(modofyValue, GetArraySplitFormat(field));
                            ...
                            if (m_currentData.TableIDs[index] != valueStr)
                            {
                                ...
                                if (repeat || isNull)
                                {
                                    if (repeat)
                                    {
                                        for (int m = 0; m < m_currentData.TableIDs.Count; m++)
                                        {
                                            Debug.Log(" index :" + index + " m:" + m + " item:" +
                                                      m_currentData.TableIDs[m]);
                                        }
    ...
    private string GetEnumType(FieldType fieldValueType, string field)
    {
        string enumType = fieldValueType == FieldType.Enum
            ? (m_currentData.m_tableEnumTypes.ContainsKey(field)
                ? m_currentData.m_tableEnumTypes[field]
                : EditorTool.GetAllEnumType()[0])
            : "";
        return enumType;
    }
    ...
    private void DrawTableConfigFieldInfo(TableConfigFieldInfo temp)
    {
        ...
        temp = (TableConfigFieldInfo)EditorDrawGUIUtil.DrawClassData("字段信息", temp,
            new List<string>() { "defultValue", "enumType" }, null, () =>
            {
    ...
    private object DrawPrefabGUI(string text, object value)
    {
        ...
        if (GUILayout.Button("o", GUILayout.Width(20)))
        {
            ObjectSelectorWindow.Show(GeneralDataModificationWindow.GetInstance(), value.ToString(),
                ...
                typeof(GameObject),
                (assetName, obj) => { editPrefabValue = assetName; }
            );
        }
    ...
    private object DrawTextureGUI(string text, object value)
    {
        ...
        if (GUILayout.Button("o", GUILayout.Width(20)))
        {
            ObjectSelectorWindow.Show(GeneralDataModificationWindow.GetInstance(), value.ToString(),
                new string[] { "Assets/Resources" },
                typeof(Texture),
                (assetName, obj) => { editTextureValue = assetName; }
            );
        }
    ...
    private void ModificationCompleteCallBack(object t)
    {
        if (t is TableConfigFieldInfo)
        {
            ...
            m_currentData.m_defaultValue[field] =
                DataConfigUtils.ObjectValue2TableString(temp.defultValue, GetArraySplitFormat(field));
        ...
        GeneralDataModificationWindow.OpenWindow(editorWindow, "插入一行数据", "", (value) =>
            {
                ...
                GeneralDataModificationWindow.otherParameter =
                    EditorDrawGUIUtil.DrawBaseValue("Insert Index:", GeneralDataModificationWindow.otherParameter);
                GeneralDataModificationWindow.otherParameter =
                    Mathf.Clamp((int)GeneralDataModificationWindow.otherParameter, 0, m_currentData.Count);
                string key = value.ToString();
                if (string.IsNullOrEmpty(key))
                    EditorGUILayout.HelpBox("Key不能为空！！", MessageType.Error);
                else if (key.Contains(" "))
    ...
    private void AddField(DataTable table, TableConfigFieldInfo info)
    {
        ...
        table.m_defaultValue.Add(info.fieldName,
            DataConfigUtils.ObjectValue2TableString(info.defultValue, info.m_ArraySplitFormat));

        foreach (var item in table.Values)
        {
            item.Add(info.fieldName,
                DataConfigUtils.ObjectValue2TableString(info.defultValue, info.m_ArraySplitFormat));
        }
    }
...
{
    [ShowGUIName("字段名")] public string fieldName = "";
    [ShowGUIName("描述")] public string description = "";
    [ShowGUIName("数据类型")] public FieldType fieldValueType;
    [ShowGUIName("数据用途")] public DataFieldAssetType fieldAssetType;
    [ShowGUIName("默认值")] public object defultValue = null;
