    ...
    public class PocoJsonSerializerStrategy : IJsonSerializerStrategy
    {
        ...
        public PocoJsonSerializerStrategy()
        {
            this.ConstructorCache =
                new ReflectionsUtils.ThreadSafeDictionary<Type, ReflectionsUtils.ConstructorDelegate>(
                    new ReflectionsUtils.ThreadSafeDictionaryValueFactory<Type, ReflectionsUtils.ConstructorDelegate>(
                        this.ContructorDelegateFactory));
            this.GetCache =
                new ReflectionsUtils.ThreadSafeDictionary<Type, IDictionary<string, ReflectionsUtils.GetDelegate>>(
                    new ReflectionsUtils.ThreadSafeDictionaryValueFactory<Type,
                        IDictionary<string, ReflectionsUtils.GetDelegate>>(this.GetterValueFactory));
            this.SetCache =
                new ReflectionsUtils.ThreadSafeDictionary<Type,
                    IDictionary<string, KeyValuePair<Type, ReflectionsUtils.SetDelegate>>>(
                    new ReflectionsUtils.ThreadSafeDictionaryValueFactory<Type,
                        IDictionary<string, KeyValuePair<Type, ReflectionsUtils.SetDelegate>>>(
        ...
        {
            return ReflectionsUtils.GetContructor(key,
                (!key.IsArray)
                    ? PocoJsonSerializerStrategy.EmptyTypes
                    : PocoJsonSerializerStrategy.ArrayConstructorParameterTypes);
        }

        internal virtual IDictionary<string, ReflectionsUtils.GetDelegate> GetterValueFactory(Type type)
        {
            IDictionary<string, ReflectionsUtils.GetDelegate> dictionary =
                new Dictionary<string, ReflectionsUtils.GetDelegate>();
            foreach (PropertyInfo current in ReflectionsUtils.GetProperties(type))
            {
                if (current.CanRead && current.CanWrite)
                {
                    ...
                    {
                        dictionary[this.MapClrMemberNameToJsonFieldName(current.Name)] =
                            ReflectionsUtils.GetGetMethod(current);
                    }
                }
            }
            ...
            foreach (FieldInfo current2 in ReflectionsUtils.GetFields(type))
            {
                ...
                {
                    dictionary[this.MapClrMemberNameToJsonFieldName(current2.Name)] =
                        ReflectionsUtils.GetGetMethod(current2);
                }
            }
        ...
        {
            IDictionary<string, KeyValuePair<Type, ReflectionsUtils.SetDelegate>> dictionary =
                new Dictionary<string, KeyValuePair<Type, ReflectionsUtils.SetDelegate>>();
            foreach (PropertyInfo current in ReflectionsUtils.GetProperties(type))
            {
                if (current.CanWrite)
                {
                    ...
                    {
                        dictionary[this.MapClrMemberNameToJsonFieldName(current.Name)] =
                            new KeyValuePair<Type, ReflectionsUtils.SetDelegate>(current.PropertyType,
                                ReflectionsUtils.GetSetMethod(current));
                    }
                }
            }
            ...
            foreach (FieldInfo current2 in ReflectionsUtils.GetFields(type))
            {
                ...
                {
                    dictionary[this.MapClrMemberNameToJsonFieldName(current2.Name)] =
                        new KeyValuePair<Type, ReflectionsUtils.SetDelegate>(current2.FieldType,
                            ReflectionsUtils.GetSetMethod(current2));
                }
            }
            ...
            else
            {
                ...
                if (text != null)
                {
                    if (text.Length != 0)
                    {
                        ...
                        if (type == typeof(DateTime) || (ReflectionsUtils.IsNullableType(type) &&
                                                         Nullable.GetUnderlyingType(type) == typeof(DateTime)))
                        {
                            result = DateTime.ParseExact(text, PocoJsonSerializerStrategy.Iso8601Format,
                        ...
                        if (type == typeof(DateTimeOffset) || (ReflectionsUtils.IsNullableType(type) &&
                                                               Nullable.GetUnderlyingType(type) ==
                                                               typeof(DateTimeOffset)))
                        {
                            result = DateTimeOffset.ParseExact(text, PocoJsonSerializerStrategy.Iso8601Format,
                                CultureInfo.InvariantCulture,
                                DateTimeStyles.AdjustToUniversal | DateTimeStyles.AssumeUniversal);
                            return result;
                        }

                        if (type == typeof(Guid) || (ReflectionsUtils.IsNullableType(type) &&
                    ...
                    else
                    {
                        ...
                        else if (ReflectionsUtils.IsNullableType(type) &&
                                 Nullable.GetUnderlyingType(type) == typeof(Guid))
                        {
                            obj = null;
                        }
                ...
                {
                    obj = ((!typeof(IConvertible).IsAssignableFrom(type))
                        ? value
                ...
                else
                {
                    ...
                    if (dictionary != null)
                    {
                        ...
                        else
                        {
                            ...
                            foreach (KeyValuePair<string, KeyValuePair<Type, ReflectionsUtils.SetDelegate>> current2 in
                                     this.SetCache[type])
                            {
                                object value2;
                    ...
                    else
                    {
                        ...
                        if (list != null)
                        {
                            ...
                            else if (ReflectionsUtils.IsTypeGenericeCollectionInterface(type) ||
                                     ReflectionsUtils.IsAssignableFrom(typeof(IList), type))
                            {
        ...
        protected virtual bool TrySerializeKnownTypes(object input, out object output)
        {
            ...
            {
                output = ((DateTime)input).ToUniversalTime().ToString(PocoJsonSerializerStrategy.Iso8601Format[0],
                    CultureInfo.InvariantCulture);
            }
            else if (input is DateTimeOffset)
            {
                output = ((DateTimeOffset)input).ToUniversalTime().ToString(PocoJsonSerializerStrategy.Iso8601Format[0],
                    CultureInfo.InvariantCulture);
            }
