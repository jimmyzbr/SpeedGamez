...
public static class JsonUtils
{
    ...
    public static bool TryFromJson<T>(out T res, string json)
    {
        try
        {
            ...
            //Debug.Log("Change Value: obj" + obj + " res:" + res);
            return state;
        }
        catch (Exception e)
        {
    ...
    private static object ClassOrStructToJsonObject(object data)
    {
        ...
        for (int i = 0; i < propertys.Length; i++)
        {
            ...
            if (CheckPropertyInfo(p))
            {
                ...
                {
                    Debug.LogError(type + " property :" + p.Name + "\n" + e);
                    continue;
                }
            }
        }
    ...
    private static object ChangeJsonDataToObjectByType(Type type, object data)
    {
        ...
        else if (type.IsArray)
        {
            ...
            {
                Debug.LogError("Array无法转换类型， data：" + data.GetType().FullName + "  type.GetElementType(): " +
        ...
        {
            throw new Exception(
                "无法转换类型， type：" + type.FullName + "  valueType: " + value.GetType().FullName + "\n " + e);
        }
