...
public class HotUpdateManager
{
    ...
    private static void GetVersion(string path, ref int bigVersion, ref int smallVersion)
    {
        ...
        Dictionary<string, object> StreamVersion =
            (Dictionary<string, object>)FrameWork.Json.Deserialize(StreamVersionContent);
    ...
    static IEnumerator CheckVersion()
    {
        ...
        while (!www.isDone)
        {
            UpdateDateCallBack(HotUpdateStatusEnum.DownLoadingVersionFile,
                GetHotUpdateProgress(false, false, www.progress));
        ...
        Dictionary<string, object> ServiceVersion =
            (Dictionary<string, object>)FrameWork.Json.Deserialize(s_versionFileCache);
    ...
    static IEnumerator DownLoadFile()
    {
        ...
        while (!www.isDone)
        {
            UpdateDateCallBack(HotUpdateStatusEnum.DownLoadingManifestFile,
                GetHotUpdateProgress(true, false, www.progress));
            yield return new WaitForEndOfFrame();
        }
    ...
    static IEnumerator StartDownLoad()
    {
        ...
        for (int i = 0; i < s_downLoadList.Count; i++)
        {
            ...
            if (md5Tmp.Equals(s_downLoadList[i].md5))
            {
                ...
                //该文件已更新完毕
                UpdateDateCallBack(HotUpdateStatusEnum.Updating,
                    GetHotUpdateProgress(true, true, GetDownLoadFileProgress(i)));
            }
            else
            {
                ...
                if (www.error != null && www.error != "")
                {
                    ...
                    UpdateDateCallBack(HotUpdateStatusEnum.UpdateFail,
                        GetHotUpdateProgress(true, true, GetDownLoadFileProgress(i)));
                ...
                else
                {
                    ...
                    ResourceIOTool.CreateFile(PathTool.GetAssetsBundlePersistentPath() + "/" + s_downLoadList[i].name,
                        www.bytes);
                    RecordManager.SaveRecord(c_HotUpdateRecordName, s_downLoadList[i].name,
                        s_downLoadList[i].md5.ToString());
        ...
        ResourceIOTool.CreateFile(PathTool.GetAssetsBundlePersistentPath() + AssetsManifestManager.c_ManifestFileName,
            s_ManifestByteCache);
